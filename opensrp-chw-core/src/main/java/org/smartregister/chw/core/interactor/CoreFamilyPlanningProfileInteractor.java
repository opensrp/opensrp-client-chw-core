...
public class CoreFamilyPlanningProfileInteractor extends BaseFpProfileInteractor implements CoreFamilyPlanningMemberProfileContract.Interactor {
    ...
    public CoreFamilyPlanningProfileInteractor(Context context) {
    ...
    protected Alert getAlerts(Context context, String baseEntityID) {
        try {
            List<BaseUpcomingService> baseUpcomingServices = new ArrayList<>(new CoreFpUpcomingServicesInteractor().getMemberServices(context, toMember(FpDao.getMember(baseEntityID))));
            if (baseUpcomingServices.size() > 0) {
                ...
                return new Alert(baseEntityID, serviceName, serviceName, serviceDate != null && serviceDate.before(new Date()) ? AlertStatus.urgent : AlertStatus.normal,
                        serviceDate != null ? AbstractDao.getDobDateFormat().format(serviceDate) : null,
                        "",
                        true
                );
            }
