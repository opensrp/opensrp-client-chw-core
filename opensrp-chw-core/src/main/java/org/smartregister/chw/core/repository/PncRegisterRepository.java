...
public class PncRegisterRepository extends BaseRepository {
    ...
    public CommonPersonObject getPncCommonPersonObject(String baseEntityId) {
        ...
        try {
            ...
            if (cursor != null && cursor.getCount() > 0 && cursor.moveToFirst()) {
                personObject = Utils.context().commonrepository(TABLE_NAME).readAllcommonforCursorAdapter(cursor);
            }
        } catch (Exception e) {
            Timber.e(e);
        } finally {
    ...
    public CommonPersonObject getPncFamilyDetailsCommonPersonObject(String baseEntityId) {
        SQLiteDatabase database = getReadableDatabase();
        Cursor cursor = null;
        CommonPersonObject personObject = null;
