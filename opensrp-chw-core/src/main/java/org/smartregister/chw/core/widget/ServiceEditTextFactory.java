...
public class ServiceEditTextFactory extends ChildEditTextFactory {
    ...
    public void attachLayout(String stepName, Context context, JsonFormFragment formFragment, JSONObject jsonObject,
        ...
        if (jsonObject.has(CoreConstants.KEY.HIA_2_INDICATOR)) {
            editText.setTag(jsonObject.get(CoreConstants.KEY.HIA_2_INDICATOR));

            editText.addTextChangedListener(new HIA2ReportFormTextWatcher(formFragment, jsonObject.get(CoreConstants.KEY.HIA_2_INDICATOR).toString()));
        }
    }
}
