...
public class AncDao extends AlertDao {
    ...
    public static MemberObject getMember(String baseEntityID) {
        String sql = "select m.base_entity_id , m.unique_id , m.relational_id , m.dob , m.first_name , m.middle_name , m.last_name , m.gender , m.phone_number , m.other_phone_number , " +
                ...
                "fh.last_name family_head_last_name, fh.phone_number family_head_phone_number , mr.* from ec_family_member m inner join ec_family f on m.relational_id = f.base_entity_id " +
                "inner join ec_malaria_confirmation mr on mr.base_entity_id = m.base_entity_id left join ec_family_member fh on fh.base_entity_id = f.family_head where m.base_entity_id = '" + baseEntityID + "' ";
