...
public interface QueryConstant {
    ...
    String HIV_INDEX_CONTACT_COMMUNITY_FOLLOWUP_REFERRAL_COUNT_QUERY = "SELECT COUNT(*)\n" +
            "FROM ec_hiv_index_contact_community_followup\n" +
            "inner join ec_family_member on ec_family_member.base_entity_id = ec_hiv_index_contact_community_followup.entity_id\n" +
            "WHERE ec_family_member.is_closed = '0'\n" +
            " AND (ec_hiv_index_contact_community_followup.date_marked_as_done IS NULL OR (julianday('now', 'localtime') - julianday(ec_hiv_outcome.date_marked_as_done) <= 3))\n" +
            "  AND ec_family_member.date_removed is null\n" +
            "  AND ec_family_member.base_entity_id NOT IN (\n" +
            "    SELECT ec_pnc_danger_signs_outcome.entity_id AS base_entity_id\n" +
            "    FROM ec_pnc_danger_signs_outcome\n" +
    ...
    String TB_OUTCOME_COUNT_QUERY = "SELECT COUNT(*)\n" +
            ...
            "    FROM ec_hiv_outcome\n" +
            "    UNION ALL\n" +
            "    SELECT ec_hiv_index_contact_community_followup.entity_id AS base_entity_id\n" +
            "    FROM ec_hiv_index_contact_community_followup\n" +
            ")\n";
    ...
    String HIV_INDEX_CONTACT_COMMUNITY_FOLLOWUP_MAIN_SELECT =
            "/*HIV INDEX CONTACTS COMMUNITY FOLLOWUP REFERRALS*/\n" +
                    "SELECT ec_family_member.first_name    AS first_name,\n" +
                    "       ec_family_member.middle_name   AS middle_name,\n" +
                    "       ec_family_member.last_name     AS last_name,\n" +
                    "       ec_family_member.dob           AS dob,\n" +
                    ...
                    "       ec_family_member.base_entity_id,\n" +
                    "       ec_family_member.relational_id AS relationalid,\n" +
                    "       ec_hiv_index_contact_community_followup.id AS n_id,\n" +
                    "       ec_hiv_index_contact_community_followup.hiv_index_contact_community_followup_referral_date AS notification_date,\n" +
                    "       'HIV Index Contact Community Followup Referral' AS notification_type\n" +
                    "FROM ec_hiv_index_contact_community_followup\n" +
                    "         inner join ec_family_member on ec_family_member.base_entity_id = ec_hiv_index_contact_community_followup.entity_id\n" +
                    "WHERE ec_family_member.is_closed = '0'\n" +
                    " AND (ec_hiv_index_contact_community_followup.date_marked_as_done IS NULL OR (julianday('now', 'localtime') - julianday(ec_hiv_index_contact_community_followup.date_marked_as_done) <= 3))\n" +
                    "  AND ec_family_member.date_removed is null\n" +
                    "  AND ec_hiv_index_contact_community_followup.entity_id IN (%s)\n" +
                    "  AND ec_hiv_index_contact_community_followup.entity_id NOT IN (\n" +
                    "    SELECT ec_anc_danger_signs_outcome.entity_id AS base_entity_id\n" +
                    "    FROM ec_anc_danger_signs_outcome\n" +
                    "    UNION ALL\n" +
                    "    SELECT ec_pnc_danger_signs_outcome.entity_id AS base_entity_id\n" +
